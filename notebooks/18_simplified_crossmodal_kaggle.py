...
class Config:
    """Script 18: Simplified Cross-Modal Attention config."""
    ...
    OUTPUT_DIR = "outputs_simplified_crossmodal"
...
print(f"  SETTINGS: d_model={cfg.CROSS_ATTN_D_MODEL}, pos_embed={cfg.USE_SHAPE_POS_EMBED}, "
      f"attn_pool={cfg.USE_ATTENTION_POOLING}, ema={cfg.USE_EMA}")
print(f"  OPTIMIZATION: backbone_lr={cfg.BACKBONE_LR}, head_lr={cfg.HEAD_LR}, loss={cfg.LOSS_TYPE}")
...
model = SimplifiedCrossModalClassifier(
...
TITLE_PREFIX = "Simplified Cross-Modal Attention (DNABERT-2 + DNAshape)"
...
def plot_training_curves(history, save_dir):
    ...
    plt.tight_layout()
    path = os.path.join(save_dir, "simplified_training_curves.png")
...
def plot_confusion_matrix(all_targets, all_preds, class_names, save_dir):
    ...
    plt.tight_layout()
    path = os.path.join(save_dir, "simplified_confusion_matrix.png")
    plt.savefig(path, dpi=150, bbox_inches="tight"); plt.close()
...
def plot_roc_curves(all_targets, all_probs, class_names, save_dir):
    ...
    plt.tight_layout()
    path = os.path.join(save_dir, "simplified_roc_curves.png")
    plt.savefig(path, dpi=150, bbox_inches="tight"); plt.close()
...
def plot_precision_recall_curves(all_targets, all_probs, class_names, save_dir):
    ...
    plt.tight_layout()
    path = os.path.join(save_dir, "simplified_pr_curves.png")
    plt.savefig(path, dpi=150, bbox_inches="tight"); plt.close()
...
def plot_per_class_metrics_bar(all_targets, all_preds, class_names, save_dir):
    ...
    plt.tight_layout()
    path = os.path.join(save_dir, "simplified_per_class_metrics.png")
    plt.savefig(path, dpi=150, bbox_inches="tight"); plt.close()
...
print("\n" + "=" * 60)
print("PIPELINE SUMMARY -- Simplified Cross-Modal Attention (Script 18)")
print("=" * 60)
print(f"  DNABERT-2:         Last {cfg.UNFREEZE_LAST_N_LAYERS} layers fine-tuned (constant LR)")
