#!/usr/bin/env python3
"""
Auto-refactored script: 16_unified_crossmodal_kaggle.py
...
class Config:
    ...
    OUTPUT_DIR = "outputs_unified_crossmodal"
...
print(f"  ANTI-OVERFIT: auto_max_len={cfg.AUTO_MAX_LENGTH}, "
      f"layerwise_lr_decay={cfg.USE_LAYERWISE_LR_DECAY}, "
      f"dropout={cfg.FUSION_DROPOUT}, ema={cfg.USE_EMA}")
print(f"  LOW-RISK: shape_pos_embed={cfg.USE_SHAPE_POS_EMBED}, "
      f"attn_pool={cfg.USE_ATTENTION_POOLING}, "
      f"loss={cfg.LOSS_TYPE}(gamma={cfg.FOCAL_GAMMA})")
print(f"  DEFERRED (off): drop_path={cfg.USE_DROP_PATH}, rdrop={cfg.USE_RDROP}")
...
class UnifiedCrossModalClassifier(nn.Module):
    def __init__(self, backbone, embedding_dim=768,
                 ...
                 use_drop_path=False, drop_path_rate=0.0,
                 use_shape_pos_embed=True, use_attention_pooling=True,
                 head_type="mlp", hidden_dim=256, kan_degree=3,
                 num_classes=4, fusion_dropout=0.4):
        super().__init__()
        ...
        if use_drop_path and num_cross_layers > 1:
            ...
            dpr = [drop_path_rate if use_drop_path else 0.0] * num_cross_layers
        self.cross_attention_layers = nn.ModuleList([
            CrossModalAttentionLayer(d_model=d_model, nhead=nhead,
        ...
        if head_type == "kan":
            self.classifier = nn.Sequential(
                ...
                ChebyKANLinear(fusion_dim, hidden_dim, degree=kan_degree),
...
model = UnifiedCrossModalClassifier(
    ...
    use_drop_path=cfg.USE_DROP_PATH,
...
print(f"\n{'='*60}")
print("MODEL SUMMARY -- Unified Cross-Modal Attention (Script 16)")
print(f"{'='*60}")
...
TITLE_PREFIX = "Unified Cross-Modal Attention (DNABERT-2 + DNAshape)"
...
def plot_training_curves(history, save_dir):
    ...
    plt.tight_layout()
    path = os.path.join(save_dir, "unified_training_curves.png")
...
def plot_confusion_matrix(all_targets, all_preds, class_names, save_dir):
    ...
    plt.tight_layout()
    path = os.path.join(save_dir, "unified_confusion_matrix.png")
    plt.savefig(path, dpi=150, bbox_inches="tight"); plt.close()
...
def plot_roc_curves(all_targets, all_probs, class_names, save_dir):
    ...
    plt.tight_layout()
    path = os.path.join(save_dir, "unified_roc_curves.png")
    plt.savefig(path, dpi=150, bbox_inches="tight"); plt.close()
...
def plot_precision_recall_curves(all_targets, all_probs, class_names, save_dir):
    ...
    plt.tight_layout()
    path = os.path.join(save_dir, "unified_pr_curves.png")
    plt.savefig(path, dpi=150, bbox_inches="tight"); plt.close()
...
def plot_per_class_metrics_bar(all_targets, all_preds, class_names, save_dir):
    ...
    plt.tight_layout()
    path = os.path.join(save_dir, "unified_per_class_metrics.png")
    plt.savefig(path, dpi=150, bbox_inches="tight"); plt.close()
...
print("\n" + "=" * 60)
print("PIPELINE SUMMARY -- Unified Cross-Modal Attention (Script 16)")
print("=" * 60)
