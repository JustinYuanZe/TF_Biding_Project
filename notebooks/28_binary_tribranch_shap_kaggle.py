#!/usr/bin/env python3
"""
Script 28: Binary Tri-Branch G-CMAB msCNN + Tri-Modal SHAP (self-contained)
...
This is a CLEANED, FULLY-RUNNABLE tri-branch that trains end-to-end AND runs a
correct tri-modal SHAP analysis inline (Sequence + DNAshape + Bio-features),
matching the standard of scripts 23-26.

WHAT THIS SCRIPT FIXES vs script 23 (without touching old scripts):
  * SHAP now attributes the BIO branch too (script 23 zeroed it out with a dummy).
  * Adds a dedicated Bio-feature SHAP bar (CpG O/E, GC, G4) and a modality-level
    contribution summary (Sequence vs Shape vs Bio) — true "tri-branch" SHAP.
  * BED export rewritten for the BINARY task (TP / FN / FP), not dead 4-class logic.
  * All report/plot/subset labels corrected for binary (no leftover "SP4" / "Script 19").
...
class Config:
    """Script 28: Binary Tri-Branch G-CMAB msCNN + SHAP config."""
...
def train_model(model, train_loader, test_loader, optimizer, scheduler, criterion, acc, cfg):
    if acc.is_main_process:
        print("\n" + "=" * 60)
        print("TRAINING -- Binary Tri-Branch G-CMAB msCNN (Script 28)")
        print("=" * 60)
        ...
        if val_acc > best_val_acc:
            ...
            if acc.is_main_process:
                ...
                torch.save({"epoch": epoch + 1, "model_state_dict": unwrapped.state_dict(),
                           ...
                           os.path.join(cfg.MODEL_DIR, "best_tribranch_shap.pt"))
...
best_path = os.path.join(cfg.MODEL_DIR, "best_tribranch_shap.pt")
...
def full_evaluation(model, test_loader, class_names, acc):
    ...
    if acc.is_main_process:
        ...
        with open(os.path.join(cfg.OUTPUT_DIR, "classification_report.txt"), "w") as f:
            f.write("CLASSIFICATION REPORT -- Binary Tri-Branch G-CMAB msCNN (Script 28)\n")
...
def parse_header_to_bed(header):
    """Positive headers look like 'chr5:10375416-10375517[_revcomp]'. Synthetic
    negatives ('neg_sp1_...') have no genomic coords and are skipped."""
    try:
...
TITLE_PREFIX = "Binary Tri-Branch G-CMAB (Seq + Shape + Bio)"
    ...
    try:
        ...
        if len(subset_A_indices) >= 3 and len(subset_B_indices) >= 3:
            ...
            with open(os.path.join(cfg.OUTPUT_DIR, "shap_summary.txt"), "w") as f:
                f.write("TRI-MODAL SHAP SUMMARY (Script 28)\n" + "=" * 50 + "\n")
...
if accelerator.is_main_process:
    print("\n" + "=" * 60)
    print("PIPELINE SUMMARY -- Binary Tri-Branch G-CMAB msCNN + SHAP (Script 28)")
    print("=" * 60)
