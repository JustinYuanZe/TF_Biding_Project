#!/usr/bin/env python3
"""
Auto-refactored script: 13_binary_sanity_check_kaggle.py
Refactored to align with project standards.
"""
"""
╔══════════════════════════════════════════════════════════════════════╗
║  Script 13: BINARY SANITY CHECK — Positive vs Negative             ║
║  Designed for: Kaggle GPU (T4/P100) or Google Colab                ║
║  Task: 2-class (SP1+SP2+SP4 = Positive) vs Negative               ║
╚══════════════════════════════════════════════════════════════════════╝

PURPOSE:
  Diagnostic test to verify the Dual-Branch pipeline has NO code bugs.
  
  - Merge SP1 + SP2 + SP4 → Label 1 (Positive)
  - Negative → Label 0
  - Output: FC(2) instead of FC(4)

EXPECTED RESULTS:
  ✅ Val Acc 85–95%  → Pipeline is correct; 4-class confusion is due
                       to SP1/SP2/SP4 similarity (expected behavior)
  ❌ Val Acc 50–60%  → BUG in DataLoader, label assignment, or shape
                       feature computation

ARCHITECTURE: Identical to Script 12 (Balanced Dual-Branch) except:
  - NUM_CLASSES = 2
...
def load_all_data(fasta_dir, shape_dir, shape_files):
    ...
    for cls_name, fpath in fasta_files.items():
        ...
        binary_label = binary_map[cls_name]
        print(f"  {cls_name}: {len(seqs)} sequences → Label {binary_label} ({cfg.CLASS_NAMES[binary_label]}) ({os.path.basename(fpath)})")
    ...
    print(f"  ★ Binary distribution: Negative={np.sum(all_labels==0)}, Positive={np.sum(all_labels==1)}")
    print(f"    (Ratio: 1:{np.sum(all_labels==1)/np.sum(all_labels==0):.1f})")
...
def split_data(sequences, labels, groups, shapes, test_size=0.2, seed=42):
    ...
    print(f"  Train binary dist: Neg={np.sum(y_train==0)}, Pos={np.sum(y_train==1)}")
    print(f"  Test  binary dist: Neg={np.sum(y_test==0)}, Pos={np.sum(y_test==1)}")
...
def robust_normalize_shapes(shape_train, shape_test):
    """Robust Scaler normalization per channel (same as Script 12)."""
    print("\n" + "=" * 60)
...
# ═══════════════════════════════════════════════════════════════════════
# CELL 5: DNABERT-2 Flash Attention Patch (Pure PyTorch)
# ═══════════════════════════════════════════════════════════════════════
...
class ShapeCNN(nn.Module):
    """1D CNN for DNAshape features. Input: [batch, 5, 101] → Output: [batch, 128]"""
...
class BalancedDualBranchClassifier(nn.Module):
    """Balanced Dual-Branch — identical to Script 12 but num_classes configurable."""
    def __init__(self, backbone, embedding_dim=768, shape_cfg=None,
                 bert_proj_dim=256, bert_dropout=0.5, shape_dropout=0.1,
                 hidden_dim=256, num_classes=2, fusion_dropout=0.3):
        super().__init__()
        ...
        self.shape_dropout = nn.Dropout(p=shape_dropout)
        ...
        fusion_dim = bert_proj_dim + shape_output_dim
    ...
    def forward(self, input_ids, attention_mask, shape_features):
        ...
        bert_raw = torch.cat([cls_token, mean_pooled], dim=1)
        bert_out = self.bert_projection(bert_raw)
        bert_out = self.bert_dropout(bert_out)

        shape_out = self.shape_cnn(shape_features)
        shape_out = self.shape_dropout(shape_out)

        fused = torch.cat([bert_out, shape_out], dim=1)
...
print(f"\n{'='*60}")
print("MODEL SUMMARY — Binary Sanity Check")
print(f"{'='*60}")
print(f"  ★ NUM_CLASSES = {cfg.NUM_CLASSES} (Positive vs Negative)")
print(f"  DNABERT-2 backbone (trainable): {backbone_trainable:>10,} params")
print(f"  ShapeCNN branch:                {shape_trainable:>10,} params")
print(f"  BERT Projection (1536→256):     {proj_trainable:>10,} params")
print(f"  Fusion head:                    {head_trainable:>10,} params")
print(f"  ─────────────────────────────────────────")
...
def full_evaluation(model, test_loader, class_names, device):
    ...
    print("\n" + "=" * 60)
    print("CLASSIFICATION REPORT — BINARY SANITY CHECK")
    print("=" * 60)
    ...
    print("=" * 60)
    if acc >= 0.85:
        print(f"  ✅ Val Acc = {acc:.1%} → PIPELINE IS CORRECT!")
        print(f"     The 4-class confusion (~47%) is caused by SP1/SP2/SP4")
        print(f"     similarity, NOT by a bug in the code.")
    elif acc >= 0.70:
        print(f"  ⚠️  Val Acc = {acc:.1%} → Pipeline likely OK but BERT")
        print(f"     may need more epochs or tuning for binary task.")
    else:
        print(f"  ❌ Val Acc = {acc:.1%} → POTENTIAL BUG DETECTED!")
        print(f"     Check DataLoader, label assignment, shape features.")
...
TITLE_PREFIX = "Binary Sanity Check (Positive vs Negative)"
...
def plot_training_curves(history, save_dir):
    ...
    plt.tight_layout()
    path = os.path.join(save_dir, "binary_sanity_training_curves.png")
...
def plot_confusion_matrix(all_targets, all_preds, class_names, save_dir):
    ...
    plt.tight_layout()
    path = os.path.join(save_dir, "binary_sanity_confusion_matrix.png")
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"  → Confusion matrix: {path}")


def plot_roc_curve_binary(all_targets, all_probs, save_dir):
    ...
    plt.title(f"ROC Curve — {TITLE_PREFIX}", fontsize=14, fontweight="bold")
    plt.legend(loc="lower right", fontsize=11)
    plt.grid(True, linestyle="--", alpha=0.3)
    plt.tight_layout()
    path = os.path.join(save_dir, "binary_sanity_roc_curve.png")
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
...
plot_roc_curve_binary(all_targets, all_probs, cfg.FIG_DIR)
...
print("\n" + "=" * 60)
print("PIPELINE SUMMARY — Binary Sanity Check")
print("=" * 60)
print(f"  Task:              BINARY (Positive vs Negative)")
print(f"  Positive class:    SP1 + SP2 + SP4 (merged)")
print(f"  Negative class:    Negative (unchanged)")
print(f"  Architecture:      Balanced Dual-Branch (same as Script 12)")
print(f"  NUM_CLASSES:       {cfg.NUM_CLASSES}")
