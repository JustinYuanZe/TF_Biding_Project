#!/usr/bin/env python3
"""
Script 30: Robustness — Chromosome-Holdout Split + Multi-Seed (self-contained)
...
======================
Scripts 23-29 evaluate the proven G-CMAB tri-branch with a GroupShuffleSplit
that keeps each (original, revcomp) pair together but otherwise splits at
random. Random splitting can still leak signal across the train/test boundary
because genomically adjacent / homologous loci (which often co-occur on the
same chromosome) can land on both sides. This script adds the two robustness
experiments the project was missing, WITHOUT touching any proven script:

  (1) CHROMOSOME-HOLDOUT SPLIT  — entire chromosomes are reserved for test, so
      no chromosome is ever in both train and test. This is the standard,
      stricter genomics generalization protocol and removes proximity/homology
      leakage that random splitting allows. (SPLIT_MODE="chromosome", default.)

  (2) MULTI-SEED LOOP            — the full train/eval is repeated over several
      seeds against the SAME fixed held-out split, and we report mean +/- std
      for every metric, so the headline number is no longer a single lucky run.

The architecture, DNABERT-2 loading (3 strategies + flash-attn patch), dataset,
training loop and evaluation are copied FAITHFULLY from
notebooks/28_binary_tribranch_shap_kaggle.py (the proven ~96.35% trainer). The
per-seed backbone reset (snapshot pretrained weights once, reload before each
seed) follows the per-variant reset pattern in notebooks/29_binary_ablation_kaggle.py.
...
class Config:
    """Script 30: Robustness (chromosome-holdout + multi-seed) config."""
    ...
    #   "chromosome" -> hold out whole chromosomes (no genomic-proximity leakage)
    #   "group"      -> GroupShuffleSplit fallback (original behaviour of script 28)
    SPLIT_MODE = "chromosome"
    # Whole chromosomes reserved for TEST. Default chosen to yield ~18% of all
    # records as test; the realized fraction is computed and PRINTED at runtime.
    # No chromosome here may appear in train -> zero proximity leakage by design.
    TEST_CHROMS = ["chr1", "chr6", "chr7"]

    # ── GroupShuffleSplit fallback (SPLIT_MODE == "group") ──
    TEST_SIZE = 0.2
    RANDOM_SEED = 42         # split seed (fixed across all training seeds)

    # ── ROBUSTNESS: multi-seed loop ──
    SEEDS = [42, 1, 2023]
...
def set_all_seeds(seed):
    """Reset python / numpy / torch RNG state for one reproducible seed."""
    random.seed(seed)
    np.random.seed(seed)
...
print("Architecture: Binary Tri-Branch G-CMAB msCNN (Seq + Shape + Bio)")
print("Robustness:   SPLIT_MODE=%s | SEEDS=%s" % (cfg.SPLIT_MODE, cfg.SEEDS))
...
def load_shape_features(data_dir, shape_files):
    all_shapes = []
    # CRITICAL: the negative DNAshape MUST correspond to the negative FASTA actually chosen
    # (load_all_data sets shape_files["Negative"] to the paired filename). We deliberately do
    # NOT fall back to any other negative-shape file — a silent fallback caused a genomic-seq /
    # dinuc-shape mismatch in an earlier run. Require the exact paired file or fail loudly.
    ...
    if not neg_shape_path:
        raise FileNotFoundError(
            ...
            f"was not found in {data_dir}. Refusing to substitute a mismatched negative-shape file. "
            f"Extract DNAshape from the matching negative FASTA (src/extract_dnashape.py) and provide it."
        )
...
def load_all_data(fasta_dir, shape_dir, shape_files):
    ...
    # Pair the negative DNAshape file to the negative FASTA that was actually chosen, so the
    # shape branch never receives features computed from a DIFFERENT negative set.
    NEG_FASTA_TO_SHAPE = {
    ...
    shape_files = dict(shape_files)  # local copy; do not mutate caller's dict
...
# ── ROBUSTNESS HELPER: chromosome extraction (handles BOTH header formats) ──
def extract_chrom(header):
    """Extract the chromosome name from a FASTA header for BOTH formats:

      positive : 'chr8:142777203-142777304'            -> 'chr8'
      positive : 'chr8:142777203-142777304_revcomp'    -> 'chr8'   (revcomp shares chrom)
      negative : 'neg_sp2_dinuc_shuffle|chr9:87793641-87793742' -> 'chr9'

    Returns the chromosome string (e.g. 'chr8'), or None if it cannot be parsed.
    """
    if header is None:
        return None
    h = header.strip()
    # Negative format: everything after the first '|' is the genomic coordinate.
    if "|" in h:
        h = h.split("|", 1)[1]
    # Chromosome is the text before the first ':' (revcomp suffix is after the
    # coordinate, so splitting on ':' already isolates the chrom cleanly).
    if ":" in h:
        chrom = h.split(":", 1)[0]
    else:
        chrom = h
    chrom = chrom.strip()
    return chrom if chrom else None


def chromosome_holdout_split(sequences, labels, groups, shapes, headers, test_chroms):
    """Hold out ENTIRE chromosomes for test. No chromosome appears in both
    train and test, so genomically adjacent / homologous loci cannot leak
    across the split. Original+revcomp pairs share a chromosome and therefore
    stay together automatically."""
    if accelerator.is_main_process:
        print("\n" + "=" * 60)
        print("SPLITTING DATA (CHROMOSOME HOLDOUT — no proximity/homology leakage)")
        print("=" * 60)
    test_chrom_set = set(test_chroms)
    chroms = [extract_chrom(h) for h in headers]
    n_unparsed = sum(1 for c in chroms if c is None)
    if n_unparsed and accelerator.is_main_process:
        print(f"  WARNING: {n_unparsed} headers could not be parsed for chromosome (kept in TRAIN).")

    test_idx = [i for i, c in enumerate(chroms) if c in test_chrom_set]
    train_idx = [i for i, c in enumerate(chroms) if c not in test_chrom_set]

    # Hard guarantee: no chromosome leaks across the split.
    train_chroms = {chroms[i] for i in train_idx if chroms[i] is not None}
    test_chroms_realized = {chroms[i] for i in test_idx if chroms[i] is not None}
    overlap = train_chroms & test_chroms_realized
    assert not overlap, f"Chromosome leakage detected (in both train and test): {sorted(overlap)}"
    assert len(test_idx) > 0, f"No samples matched TEST_CHROMS={sorted(test_chrom_set)}"

    realized_frac = 100.0 * len(test_idx) / max(len(sequences), 1)
    if accelerator.is_main_process:
        print(f"  TEST_CHROMS requested: {sorted(test_chrom_set)}")
        print(f"  TEST_CHROMS realized:  {sorted(test_chroms_realized)}")
        print(f"  Train chromosomes:     {sorted(train_chroms)}")
        print(f"  Train: {len(train_idx)} | Test: {len(test_idx)} "
              f"(test fraction = {realized_frac:.2f}% of all records)")
        if not (15.0 <= realized_frac <= 25.0):
            print(f"  NOTE: realized test fraction {realized_frac:.2f}% is outside the "
                  f"~18-22% target; adjust cfg.TEST_CHROMS if undesired.")
    return _gather_split(sequences, labels, shapes, headers, train_idx, test_idx)


def group_shuffle_split(sequences, labels, groups, shapes, headers, test_size, seed):
    """Original behaviour of script 28: GroupShuffleSplit keeps revcomp pairs
    together but otherwise splits at random (proximity leakage possible)."""
    if accelerator.is_main_process:
        print("\n" + "=" * 60)
        print("SPLITTING DATA (GroupShuffleSplit — no revcomp leakage, random split)")
        print("=" * 60)
...
def _gather_split(sequences, labels, shapes, headers, train_idx, test_idx):
...
def split_data(sequences, labels, groups, shapes, headers, cfg):
    if cfg.SPLIT_MODE == "chromosome":
        return chromosome_holdout_split(sequences, labels, groups, shapes, headers, cfg.TEST_CHROMS)
    elif cfg.SPLIT_MODE == "group":
        return group_shuffle_split(sequences, labels, groups, shapes, headers, cfg.TEST_SIZE, cfg.RANDOM_SEED)
    else:
        raise ValueError(f"Unknown SPLIT_MODE={cfg.SPLIT_MODE!r} (expected 'chromosome' or 'group')")


all_sequences, all_labels, all_groups, all_shapes, all_headers = load_all_data(
    cfg.FASTA_DIR, cfg.SHAPE_DIR, cfg.SHAPE_FILES
)
(seq_train, seq_test, y_train_raw, y_test_raw,
 shape_train, shape_test, headers_train, headers_test) = split_data(
    all_sequences, all_labels, all_groups, all_shapes, all_headers, cfg,
)
...
# ── ROBUSTNESS: snapshot pretrained backbone ONCE; reload before each seed so
# every seed starts from the identical pretrained weights (mirrors the
# per-variant reset in notebooks/29_binary_ablation_kaggle.py). ──
INITIAL_BACKBONE_STATE = {k: v.detach().cpu().clone() for k, v in dnabert_model.state_dict().items()}
if accelerator.is_main_process:
    print(f"  Snapshotted pretrained backbone state ({len(INITIAL_BACKBONE_STATE)} tensors) for per-seed reset.")
...
class TriBranchClassifier(nn.Module):
    ...
    def _fuse_from_embeddings(self, seq_embeddings, shape_features, bio_features, attention_mask):
        """Shared trunk from precomputed seq embeddings -> logits."""
...
y_train = np.array([1 if l < 3 else 0 for l in y_train_raw])
y_test = np.array([1 if l < 3 else 0 for l in y_test_raw])

# Datasets are FIXED across seeds (same chromosome-holdout split). Only model
# init / data shuffling / dropout RNG change per seed.
...
if accelerator.is_main_process:
    print(f"\nDatasets: {len(train_dataset)} train / {len(test_dataset)} test | max_length={MAX_LENGTH}")
    print(f"  Train binary dist (neg,pos): {np.bincount(y_train)} | Test: {np.bincount(y_test)}")

# ═══════════════════════════════════════════════════════════════════════
# CELL 9: Training + Evaluation primitives (faithful to script 28)
# ═══════════════════════════════════════════════════════════════════════
...
def train_one_epoch(model, loader, optimizer, scheduler, criterion, acc, grad_accum, max_grad_norm, epoch_num):
    ...
    if epoch_num < 3 and grad_norms and acc.is_main_process:
...
@torch.no_grad()
def full_evaluation(model, test_loader, acc):
    """Return arrays of preds/targets/probs gathered across processes."""
    model.eval()
    all_preds, all_targets, all_probs = [], [], []
    for batch in tqdm(test_loader, desc="  Evaluating", disable=not acc.is_main_process, leave=False):
        with acc.autocast():
            logits = model(batch["input_ids"], batch["attention_mask"],
                           batch["shape_features"], batch["bio_features"])
...
def compute_metrics(targets, preds, probs):
    """Six headline metrics for one seed."""
    acc_val = accuracy_score(targets, preds)
    f1b = f1_score(targets, preds, average="binary", zero_division=0)
    roc = roc_auc_safe(targets, probs)
    pr = average_precision_score(targets, probs)
    neg_recall = recall_score(targets, preds, pos_label=0, zero_division=0)
    pos_recall = recall_score(targets, preds, pos_label=1, zero_division=0)
    return {
        "accuracy": acc_val, "f1_binary": f1b, "roc_auc": roc,
        "pr_auc": pr, "neg_recall": neg_recall, "pos_recall": pos_recall,
    }

# ═══════════════════════════════════════════════════════════════════════
# CELL 10: Build-and-train ONE seed (fresh model, reset backbone)
# ═══════════════════════════════════════════════════════════════════════
...
def run_one_seed(seed, cfg, acc):
    if acc.is_main_process:
        print("\n" + "#" * 60)
        print(f"# SEED {seed}  (split={cfg.SPLIT_MODE})")
        print("#" * 60)

    # 1) Reset all RNG + backbone to the snapshotted pretrained weights.
    set_all_seeds(seed)
    ...
    pos_weight = torch.tensor([pos_weight_val], dtype=torch.float32, device=acc.device)
    ...
    # 4) Train with early stopping; keep the best-val checkpoint for this seed.
    best_val_acc, patience_counter = 0.0, 0
    best_ckpt_path = os.path.join(cfg.MODEL_DIR, f"best_seed{seed}.pt")
    ...
    for epoch in range(cfg.EPOCHS):
        ...
        if acc.is_main_process:
            print(f"  [seed {seed}] Epoch {epoch+1:02d}/{cfg.EPOCHS} | Train {tr_loss:.4f}/{tr_acc:.4f} | "
                  f"Val {val_loss:.4f}/{val_acc:.4f} | Gap {gap:+.2f}% | "
                  f"LR {optimizer.param_groups[0]['lr']:.1e} | {elapsed:.0f}s")

        if gap >= cfg.MAX_OVERFITTING_GAP:
            if acc.is_main_process:
                print(f"  Early stop (overfitting gap {gap:+.2f}% >= {cfg.MAX_OVERFITTING_GAP}%)")
            break
        ...
        if val_acc > best_val_acc:
            ...
            if acc.is_main_process:
                ...
                torch.save({"epoch": epoch + 1, "model_state_dict": unwrapped.state_dict(),
                            "val_acc": val_acc, "seed": seed}, best_ckpt_path)
    ...
    # 5) Reload best checkpoint and run full evaluation for this seed.
    acc.wait_for_everyone()
    if os.path.exists(best_ckpt_path):
        ckpt = torch.load(best_ckpt_path, map_location=acc.device)
        acc.unwrap_model(model).load_state_dict(ckpt["model_state_dict"])
        if acc.is_main_process:
            print(f"  Loaded best seed-{seed} model (epoch {ckpt['epoch']}, val_acc={ckpt['val_acc']:.4f})")

    preds, targets, probs = full_evaluation(model, test_loader, acc)
    metrics = compute_metrics(targets, preds, probs) if acc.is_main_process else None

    if acc.is_main_process:
        print(f"  [seed {seed}] " + " | ".join(f"{k}={v:.4f}" for k, v in metrics.items()))

    # 6) Cleanup so the next seed starts clean.
    del model, optimizer, scheduler, train_loader, test_loader, criterion
    gc.collect()
    if torch.cuda.is_available():
...
# ═══════════════════════════════════════════════════════════════════════
# CELL 11: Multi-Seed Loop + mean±std aggregation
# ═══════════════════════════════════════════════════════════════════════

METRIC_KEYS = ["accuracy", "f1_binary", "roc_auc", "pr_auc", "neg_recall", "pos_recall"]

per_seed_rows = []   # list of dicts (main process only)
last_history = None

for seed in cfg.SEEDS:
    metrics, history = run_one_seed(seed, cfg, accelerator)
    last_history = history
    if accelerator.is_main_process and metrics is not None:
        row = {"seed": seed, "split_mode": cfg.SPLIT_MODE}
...
if accelerator.is_main_process:
    ...
    # 1) Per-seed CSV
    csv_path = os.path.join(cfg.OUTPUT_DIR, "robustness_results.csv")
    fieldnames = ["seed", "split_mode"] + METRIC_KEYS
    ...
    for k in METRIC_KEYS:
        vals = np.array([r[k] for r in per_seed_rows], dtype=float)
        summary[k] = (float(np.mean(vals)), float(np.std(vals)))

    summary_lines = []
    summary_lines.append(f"SPLIT_MODE={cfg.SPLIT_MODE} | SEEDS={cfg.SEEDS} | n_seeds={len(per_seed_rows)}")
    if cfg.SPLIT_MODE == "chromosome":
        summary_lines.append(f"TEST_CHROMS={cfg.TEST_CHROMS}")
    for k in METRIC_KEYS:
        m, s = summary[k]
        summary_lines.append(f"  {k:12s}: {m:.4f} +/- {s:.4f}")
    summary_text = "\n".join(summary_lines)
    print(summary_text)

    # Append summary rows to the CSV as well (mean / std rows for easy parsing).
    with open(csv_path, "a", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        mean_row = {"seed": "MEAN", "split_mode": cfg.SPLIT_MODE}
        std_row = {"seed": "STD", "split_mode": cfg.SPLIT_MODE}
        for k in METRIC_KEYS:
            mean_row[k] = summary[k][0]
            std_row[k] = summary[k][1]
        writer.writerow(mean_row)
        writer.writerow(std_row)

    # 3) Plain-text summary file
    summary_path = os.path.join(cfg.OUTPUT_DIR, "robustness_summary.txt")
    with open(summary_path, "w") as f:
        f.write("ROBUSTNESS SUMMARY -- Chromosome-Holdout + Multi-Seed (Script 30)\n")
        f.write("Binary Tri-Branch G-CMAB (Seq DNABERT-2 + DNAshape + Bio)\n")
        f.write("=" * 60 + "\n")
        f.write(summary_text + "\n\n")
        f.write("Per-seed:\n")
        for row in per_seed_rows:
            f.write("  seed %-6s " % str(row["seed"]) +
                    " ".join(f"{k}={row[k]:.4f}" for k in METRIC_KEYS) + "\n")
    print(f"  Summary -> {summary_path}")

    # 4) Bar chart with error bars (mean +/- std) for accuracy and negative recall
    fig, ax = plt.subplots(figsize=(8, 6))
    metrics_to_plot = ["accuracy", "neg_recall"]
    labels = ["Accuracy", "Negative recall"]
    means = [summary[k][0] for k in metrics_to_plot]
    stds = [summary[k][1] for k in metrics_to_plot]
    x = np.arange(len(metrics_to_plot))
    bars = ax.bar(x, means, yerr=stds, capsize=8, color=["#2196F3", "#FF9800"],
    ...
    ax.set_title(f"Robustness ({cfg.SPLIT_MODE} split, {len(per_seed_rows)} seeds)\n"
                 f"mean +/- std", fontweight="bold")
    ax.grid(True, linestyle="--", alpha=0.3, axis="y")
    plt.tight_layout()
    bar_path = os.path.join(cfg.FIG_DIR, "robustness_mean_std_bar.png")
    plt.savefig(bar_path, dpi=150, bbox_inches="tight"); plt.close()
    print(f"  Bar chart -> {bar_path}")

    # 5) Optional: training curves from the last seed (sanity figure)
    if last_history is not None and len(last_history["train_loss"]) > 0:
        epochs = len(last_history["train_loss"])
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5))
        ax1.plot(range(1, epochs + 1), last_history["train_loss"], label="Train Loss", color="#2196F3", linewidth=2)
        ax1.plot(range(1, epochs + 1), last_history["val_loss"], label="Val Loss", color="#FF5722", linewidth=2)
        ax1.set_title("Loss (last seed)", fontweight="bold"); ax1.set_xlabel("Epoch"); ax1.set_ylabel("Loss")
        ax1.legend(); ax1.grid(True, linestyle="--", alpha=0.4)
        ax2.plot(range(1, epochs + 1), last_history["train_acc"], label="Train Acc", color="#4CAF50", linewidth=2)
        ax2.plot(range(1, epochs + 1), last_history["val_acc"], label="Val Acc", color="#E91E63", linewidth=2)
        ax2.axhline(y=0.5, color="gray", linestyle="--", alpha=0.5, label="Chance (0.50)")
        ax2.set_title("Accuracy (last seed)", fontweight="bold"); ax2.set_xlabel("Epoch"); ax2.set_ylabel("Accuracy")
        ax2.legend(); ax2.grid(True, linestyle="--", alpha=0.4)
        plt.tight_layout()
        plt.savefig(os.path.join(cfg.FIG_DIR, "robustness_last_seed_curves.png"), dpi=150, bbox_inches="tight")
        plt.close()
        print(f"  Last-seed curves -> {os.path.join(cfg.FIG_DIR, 'robustness_last_seed_curves.png')}")
...
if accelerator.is_main_process:
    print("\n" + "=" * 60)
    print("PIPELINE SUMMARY -- Robustness (chromosome-holdout + multi-seed) (Script 30)")
    print("=" * 60)
    print(f"  Split mode:      {cfg.SPLIT_MODE}")
    if cfg.SPLIT_MODE == "chromosome":
        print(f"  Test chroms:     {cfg.TEST_CHROMS}")
    ...
    zip_filename = "outputs_robustness"
