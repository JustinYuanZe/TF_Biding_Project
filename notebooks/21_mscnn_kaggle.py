#!/usr/bin/env python3
"""
Auto-refactored script: 21_mscnn_kaggle.py
Refactored to align with project standards.
"""
"""
Script 21: G-CMAB msCNN — Multi-Scale CNN (msCNN)
...
class Config:
    ...
    OUTPUT_DIR = "outputs_gcmab_mscnn"
    ...
    # ── Cross-Modal Attention (optional/kept from Script 19) ──
    USE_CROSS_ATTN = True
    ...
    # ══════════════════════════════════════════════════════════════════
    # SCRIPT 19/20 FLAGS — adapted for msCNN
    # ══════════════════════════════════════════════════════════════════
    ...
    HEAD_TYPE = "mlp"
    HIDDEN_DIM = 256
    FUSION_DROPOUT = 0.7  # FAD paper recommended dropout 0.7
    HEAD_LR = 2e-4
...
print(f"Architecture: G-CMAB msCNN (Script 21)")
print(f"  Base: Script 19 (d_model={cfg.CROSS_ATTN_D_MODEL}, 1 cross-attn, weighted-CE)")
print(f"  Cross-Attention: {cfg.USE_CROSS_ATTN}")
print(f"  Seq msCNN kernels: {cfg.SEQ_MSCNN_KERNELS} (out={cfg.MSCNN_OUT_CHANNELS})")
print(f"  Shape msCNN kernels: {cfg.SHAPE_MSCNN_KERNELS} (out={cfg.MSCNN_OUT_CHANNELS})")
print("=" * 60)
...
# ═══════════════════════════════════════════════════════════════════════
# CELL 7: Architecture — G-CMAB msCNN
# ═══════════════════════════════════════════════════════════════════════
...
# ---------- Component 2: Depthwise Separable Convolutions & msCNN Blocks ----------

class DepthwiseSeparableConv1d(nn.Module):
    """
    Depthwise Separable Convolution in 1D.
    Separates spatial convolution from channel-wise mixing to reduce parameters.
    """
...
class MSCNNBranchStack(nn.Module):
    """
    Parallel stack of Depthwise Separable convolutions with different kernel sizes.
    Each branch applies 1-Max Pooling along the spatial dimension.
    """
    ...
    def forward(self, x):
        ...
        for branch in self.branches:
            conv_out = branch(x)  # [B, out_channels, L]
            # 1-Max Pooling along spatial dimension (dim=2)
            pooled = conv_out.max(dim=2)[0]  # [B, out_channels]
            pooled_outputs.append(pooled)
        return torch.cat(pooled_outputs, dim=1)  # [B, len(kernels) * out_channels]


# ---------- Component 3: Cross-Modal Attention ----------
...
# ---------- Component 4: Main Classifier (G-CMAB msCNN) ----------

class GCMABmsCNNClassifier(nn.Module):
    """
    Script 21: G-CMAB msCNN Classifier.
    Combines DNABERT-2 and DNAshape features, uses optional Cross-Modal Attention,
    and aggregates features using multi-scale Depthwise Separable Convolutions (msCNN)
    and 1-Max Pooling.
    """
    def __init__(self, backbone, cfg):
        ...
        # ── Layer-Attention over BERT hidden states ──
        ...
        if self.use_cross_attn:
            # Branch 2: DNAshape projection to map 5 channels to d_model preserving length 101
            ...
            # msCNN stacks operating on d_model channels
            self.seq_mscnn = MSCNNBranchStack(
                in_channels=d_model,
                out_channels=cfg.MSCNN_OUT_CHANNELS,
                kernels=cfg.SEQ_MSCNN_KERNELS,
            )
            self.shape_mscnn = MSCNNBranchStack(
                in_channels=d_model,
                out_channels=cfg.MSCNN_OUT_CHANNELS,
                kernels=cfg.SHAPE_MSCNN_KERNELS,
            )
        else:
            # Bypassing Cross-Modal Attention:
            self.seq_mscnn = MSCNNBranchStack(
                in_channels=d_model,
                out_channels=cfg.MSCNN_OUT_CHANNELS,
                kernels=cfg.SEQ_MSCNN_KERNELS,
        ...
        in_features = (len(cfg.SEQ_MSCNN_KERNELS) + len(cfg.SHAPE_MSCNN_KERNELS)) * cfg.MSCNN_OUT_CHANNELS
    ...
    def forward(self, input_ids, attention_mask, shape_features):
        ...
        if self.use_cross_attn:
            shape_proj = self.shape_projection(shape_features) # [B, d_model, 101]
            shape_feats = shape_proj.transpose(1, 2) # [B, 101, d_model]
            ...
            seq_in = seq_features.transpose(1, 2) # [B, d_model, T]
            shape_in = shape_feats.transpose(1, 2) # [B, d_model, 101]
        else:
            seq_in = seq_features.transpose(1, 2)
            shape_in = shape_features  # [B, 5, 101]

        seq_pooled = self.seq_mscnn(seq_in)     # [B, len(cfg.SEQ_MSCNN_KERNELS) * MSCNN_OUT_CHANNELS]
        shape_pooled = self.shape_mscnn(shape_in) # [B, len(cfg.SHAPE_MSCNN_KERNELS) * MSCNN_OUT_CHANNELS]
...
model = GCMABmsCNNClassifier(backbone=dnabert_model, cfg=cfg)
...
if hasattr(model, "shape_projection") and model.shape_projection is not None:
    param_groups.append({"params": list(model.shape_projection.parameters()), "lr": cfg.SHAPE_LR, "weight_decay": cfg.WEIGHT_DECAY})

if hasattr(model, "cross_attention_layers") and model.cross_attention_layers is not None:
...
if accelerator.is_main_process:
    print(f"\n{'='*60}")
    print("MODEL SUMMARY -- G-CMAB msCNN (Script 21)")
    print(f"{'='*60}")
    ...
    print(f"  Cross-Attention:     {cfg.USE_CROSS_ATTN}")
    print(f"  GroupNorm:           {cfg.USE_GROUPNORM}")
    print(f"  Layer-Attention:     {cfg.USE_LAYER_ATTN} (N={cfg.LAYER_ATTN_N}, LR={cfg.LAYER_ATTN_LR})")
    print(f"  Seq msCNN kernels:   {cfg.SEQ_MSCNN_KERNELS} (out={cfg.MSCNN_OUT_CHANNELS})")
    print(f"  Shape msCNN kernels: {cfg.SHAPE_MSCNN_KERNELS} (out={cfg.MSCNN_OUT_CHANNELS})")
    ...
    for epoch in range(cfg.EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            if accelerator_obj.is_main_process:
                ...
                }, os.path.join(cfg.MODEL_DIR, "best_gcmab_mscnn.pt"))
...
best_path = os.path.join(cfg.MODEL_DIR, "best_gcmab_mscnn.pt")
...
TITLE_PREFIX = "G-CMAB msCNN (DNABERT-2 + DNAshape)"
...
def plot_training_curves(history, save_dir):
    ...
    plt.tight_layout()
    path = os.path.join(save_dir, "gcmab_mscnn_training_curves.png")
...
def plot_confusion_matrix(all_targets, all_preds, class_names, save_dir):
    ...
    plt.tight_layout()
    path = os.path.join(save_dir, "gcmab_mscnn_confusion_matrix.png")
    plt.savefig(path, dpi=150, bbox_inches="tight"); plt.close()
...
def plot_roc_curves(all_targets, all_probs, class_names, save_dir):
    ...
    plt.tight_layout()
    path = os.path.join(save_dir, "gcmab_mscnn_roc_curves.png")
    plt.savefig(path, dpi=150, bbox_inches="tight"); plt.close()
...
def plot_precision_recall_curves(all_targets, all_probs, class_names, save_dir):
    ...
    plt.tight_layout()
    path = os.path.join(save_dir, "gcmab_mscnn_pr_curves.png")
    plt.savefig(path, dpi=150, bbox_inches="tight"); plt.close()
...
if accelerator.is_main_process:
    print("\n" + "=" * 60)
    print("PIPELINE SUMMARY -- G-CMAB msCNN (Script 21)")
    print("=" * 60)
    ...
    print("  |  Script 19 (G-CMAB Safe):     61.51%  (best val acc)      |")
    print(f"  |  Script 21 (G-CMAB msCNN):   {max(history['val_acc']):.2%}                      |")
    print("  +-----------------------------------------------------------+")
    print("=" * 60)
        ...
        if len(subset_A_indices) > 0 and len(subset_B_indices) > 0:
            ...
            class ModelEmbeddingWrapper(nn.Module):
                ...
                def forward(self, seq_embeddings, shape_features):
                    seq_features = self.gcmab_model.seq_projection(seq_embeddings) # [B, T, d_model]
                    
                    if self.gcmab_model.use_cross_attn:
                        shape_proj = self.gcmab_model.shape_projection(shape_features) # [B, d_model, 101]
                        shape_feats = shape_proj.transpose(1, 2) # [B, 101, d_model]
            ...
            shape_bar_path = os.path.join(cfg.FIG_DIR, "gcmab_mscnn_shap_dnashape_bar.png")
            ...
            if avg_aligned_A is not None and avg_aligned_B is not None:
                ...
                heatmap_path = os.path.join(cfg.FIG_DIR, "gcmab_mscnn_shap_sequence_heatmap.png")
                ...
                line_path = os.path.join(cfg.FIG_DIR, "gcmab_mscnn_shap_sequence_line.png")
...
if accelerator.is_main_process:
    ...
    zip_filename = "outputs_gcmab_mscnn"
