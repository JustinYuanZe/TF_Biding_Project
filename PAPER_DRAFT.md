...
(i) A complete data preprocessing pipeline, including the entire process of extracting data from ENCODE, balancing data, and creating composition-preserving, motif-free negative and zero-data-leakage negative datasets.
...
Under the binary formulation, evaluated against matched dinucleotide-shuffled negatives, the multimodal G-CMAB model reaches 81.4% accuracy (macro-F1 0.751, ROC-AUC 0.856; Table 2), modestly above the fine-tuned sequence-only DNABERT-2 (78.7%, ROC-AUC 0.817). The improvement over sequence-only is small and concentrated in negative rejection (Negative recall 0.614 vs 0.585) and ranking; at the default 0.50 threshold the model still over-predicts the positive class, and threshold tuning recovers further Negative recall (closing note). An earlier 96.35% reading of this model was traced to a sequence–shape pairing bug — genomic DNAshape inadvertently paired with dinucleotide sequences, which let the shape branch separate the classes unrealistically — and is not used. Section 4.4 and Section 5.2 examine which components contribute.
...
| Sequence-only DNABERT-2 (24) | 0.7871 | 0.7189 | 0.576 | 0.585 | 0.860 | 0.855 |
| Classical TF-IDF · Logistic Regression (27)‡ | 0.8073 | 0.6874 | 0.720 | 0.376 | 0.820 | 0.951 |
| **Proposed multimodal G-CMAB (28)** | **0.8144** | **0.7508** | **0.636** | **0.614** | **0.872** | **0.882** |

(‡ Classical baselines were run by `notebooks/27_*`, which defaults to the genomic GC-matched negatives; on the dinucleotide negatives the best classical model — TF-IDF logistic regression — is essentially identical at 80.55 %. Deep rows are verified clean-dinucleotide runs, `figures/rerun/outputs_*_dinuc`. ROC-AUC: sequence-only 0.817, multimodal 0.856. The earlier 0.9635 row was a sequence–shape pairing artifact and is withdrawn.)

### 4.4 Controlled ablation
The 27-script development history (Section 5.3) was an informal architecture search, so it cannot attribute performance to any single component. To do that we ran a controlled ablation: one configurable model trained under a fixed data split, seed, and budget (15 epochs), with a single component toggled at a time (`notebooks/29_binary_ablation_kaggle.py`, output `figures/outputs_ablation_no_ckpt/ablation_results.csv`). **Table 3 below is from the earlier genomic-shape pairing and is withdrawn for the same artifact reason as the headline.** A clean 15-epoch ablation on the matched dinucleotide data collapsed to the majority class (Negative recall ≈ 0, ROC-AUC ≈ 0.52), showing that 15 epochs is too few once the artifact is removed; a clean 30-epoch re-run (`ABLATION_EPOCHS=30`) is in progress and will replace it. The component reading below is therefore provisional.
...
A 101 bp window holds enough information to decide whether an SP-family factor binds, but not which paralog binds, because SP1, SP2, and SP4 share an almost identical GC-box and overlapping shape profiles. Moving to binding-vs-non-binding and using the full multimodal model gives 81.4% accuracy on the clean dinucleotide benchmark — modestly above the sequence-only DNABERT-2 (78.7%) and the best classical baseline (TF-IDF logistic regression, ~80.6%). The multimodal edge is real but small, and it holds across negative types: the same model scores a consistent 79.6% against real GC-matched genomic negatives (Section 5.2). Section 5.2 then asks which components are responsible, a more careful answer than crediting shape or attention on their own.

### 5.2 What actually drives the gain
The ablation complicates the intuitive "shape via attention" story. Cross-modal attention gives a real but modest ranking improvement (ROC-AUC 0.650→0.701), DNAshape adds a small accuracy gain, and the large jump in accuracy and specificity appears once the bio-feature branch is present (A3→A4). The problem is that the three bio features carry almost no discriminative information between positives and their dinucleotide-shuffled negatives. The Altschul–Erickson shuffle preserves mono- and dinucleotide counts exactly, so CpG O/E (0.699 for positives vs 0.694 for negatives) and GC content (0.651 vs 0.651) are nearly identical between the two classes, and G-quadruplex motifs are rare in both (4.8% vs 3.2%). The most likely reading, then, is that the bio branch helps as an optimization or regularization term rather than as a source of biological signal: with the branch attached, the network settles into a lower validation-loss basin within the same budget (about 0.20, against about 0.30 without it). This was not what we expected, and it means the original framing — that DNAshape via cross-modal attention is the primary driver — is not supported by the ablation. The honest reading on this benchmark is that DNAshape and cross-modal attention provide the genuine, if modest, discriminative gains, while the bio branch contributes mainly as a regularizer. Confirming the effect with multiple seeds, and testing whether it survives a harder negative set, are part of the follow-up work described in Section 6.
...
### 5.4 Limitations
Several limitations bound the present claims. First, the data come from a single cell line (HepG2) and a single TF family (the three SP paralogs), so generalization to other cell types and families is untested. Second, absolute performance is modest: 81.4% is only a few points above the all-positive baseline and roughly level with a classical TF-IDF logistic regression, so the multimodal architecture is not yet clearly worth its complexity, and improving it (branch upgrades, threshold/loss calibration for Negative recall) is the focus of ongoing work. Third, the headline split is a random group split that keeps each sequence with its reverse complement, which removes augmentation leakage but not genomic-proximity or homology leakage; a chromosome-holdout, multi-seed run (`notebooks/30_*`) gives a consistent 79.4% ± 0.8% on genomic negatives, so the result is stable to both negative type and split. Fourth, the controlled ablation uses a single seed at a 15-epoch budget, so the component effects warrant multi-seed confirmation. Fifth, paralog identification is still unsolved at 101 bp resolution.

### 5.5 Performance ceiling
Binary accuracy plateaus near 81% and is consistent across negative types (about 80% on real genomic negatives), suggesting a 101 bp window holds only so much of what the task needs once sequence, shape, and biochemical context are all used. Pushing past it will probably need longer context, a chromatin or accessibility signal such as ATAC-seq, or quantitative binding affinities in place of binary peak calls — alongside the operating-point and branch-level improvements already under way.

---

## 6. Conclusion
We built a multimodal tri-branch model (G-CMAB) that predicts SP-family TF binding by fusing DNABERT-2 sequence embeddings, DNAshape geometry, and biochemical features through bidirectional cross-modal attention and a multi-scale CNN. After moving from per-paralog to binding-vs-non-binding classification — a change the data forced rather than one we chose freely — the model reaches 81.4% accuracy on a leakage-controlled dinucleotide benchmark (a consistent ~80% on real genomic negatives), modestly above a sequence-only transformer, and stays interpretable throughout via SHAP and BED-track export. Diagnosing and correcting a DNAshape pairing artifact — which had inflated the figure to 96% — is itself a cautionary contribution. The clearest next steps are lifting the modest absolute performance through operating-point calibration and branch upgrades, then generalization to additional TF families and cell types, chromatin-accessibility signal, and paralog discrimination with longer genomic context.
...
Every quantitative result above is copied from a saved report in the repository. Status of the key numbers: (1) **Classical baselines** — best is TF-IDF logistic regression, 80.73 % on genomic / 80.55 % on dinucleotide negatives (`notebooks/27_*`). (2) **Sequence-only DNABERT-2** — verified clean runs give 78.71 % (dinucleotide) / 78.93 % (genomic). (3) **The proposed model's 96.35 % has been DISPROVEN and withdrawn.** A clean, sequence–shape-matched re-run gives **81.44 %** on dinucleotide negatives and 79.6 % on genomic (`figures/rerun/outputs_tribranch_shap_dinuc` / `outputs_tribranch_shap`). The original 96.35 % paired genomic DNAshape with dinucleotide sequences, letting the shape branch separate the classes unrealistically; `outputs_real_negatives/artifact_check.txt` records the inflation. (4) Exclusive-peak counts are the on-disk 6,815 / 1,696 / 8,914. (5) **Consistency.** Once the pairing is fixed, the dinucleotide/genomic distinction collapses: the model is stable at ~79–81 % (multimodal), ~79 % (sequence-only), ~80.6 % (classical) across both negative types. (6) **Pending clean re-runs (Vast):** the controlled ablation at 15 epochs collapsed to the majority class once the artifact was removed, so Table 3 is withdrawn pending a 30-epoch re-run (`ABLATION_EPOCHS=30`); recall calibration (threshold tuning, `POS_WEIGHT_SCALE`) and branch-capacity knobs (`CROSS_ATTN_LAYERS`, `UNFREEZE_LAST_N`) were added to `notebooks/28_*` for the improvement runs. (7) Resolve the `[VERIFY]` citations in `PAPER_SUPPLEMENT.md`.
