...
Under the binary formulation the multimodal G-CMAB model reaches 96.35% accuracy and macro-F1 0.9501 (Table 2), against 76.24% for the fine-tuned sequence-only DNABERT-2. Most of the gain is in negative rejection. Negative recall rises from 0.324 in the sequence-only model to 0.886, and Negative precision from 0.547 to 0.966; SP-Positive recall reaches 0.990. The pattern holds across every sequence-only method we tried: the fine-tuned transformer (76.2%) and the best classical k-mer model (80.7%) both stall at a Negative recall of 0.32–0.38, while the full multimodal model reaches 0.886. Section 4.4 and Section 5.2 work through which components account for this difference, and the answer is less tidy than "shape plus attention".
...
A 101 bp window holds enough information to decide whether an SP-family factor binds, but not which paralog binds, because SP1, SP2, and SP4 share an almost identical GC-box and overlapping shape profiles. Moving to binding-vs-non-binding and using the full multimodal model raises binary accuracy from 76.2% (sequence-only DNABERT-2) and 80.7% (the best classical model, TF-IDF logistic regression) to 96.35%. Once again the main effect is better negative rejection: Negative recall climbs from the 0.32–0.38 ceiling shared by the sequence-only methods to 0.886. Section 4.4 and Section 5.2 say which components are responsible, and the story is more careful than crediting shape or attention on their own.
...
Three checks are needed before reading any biology into this effect: multiple random seeds, no-bio variants trained to full convergence, and re-evaluation against real genomic negatives rather than shuffles. Two are now in hand. The separability analysis (`notebooks/analyze_separability.py`) shows that against real genomic negatives CpG O/E does carry signal (univariate ROC-AUC 0.695, against about 0.50 for dinucleotide negatives), so the bio branch is informative for genuine negatives even though it is not for shuffles. The chromosome-holdout, multi-seed robustness run on the genomic negatives (`notebooks/30_*`, test chromosomes chr1/6/7, seeds 42/1/2023) reaches 95.24% ± 0.93% accuracy (ROC-AUC 0.971, Negative recall 0.850), about 1.1 pp below the random-split headline, which suggests the result is stable to leakage and not an artifact of the shuffle. One data issue affected that run: the genomic negative sequences were paired with the dinucleotide negatives' shape tensor. The loaders now require the negative sequence and its shape file to come from the same set, and refuse any mismatched fallback; the verified genomic dataset is in place (`data/processed/FINAL`), and the run will be repeated on it with the matched genomic DNAshape. The full-convergence ablation control (`notebooks/31_*`) is ready to run.
...
### 5.4 Limitations
Several limitations bound the present claims. First, the data come from a single cell line (HepG2) and a single TF family (the three SP paralogs), so generalization to other cell types and families is untested. Second, the headline result uses a random group split that keeps each sequence with its reverse complement. This removes augmentation leakage but not genomic-proximity or homology leakage; a chromosome-holdout split addresses that, and the multi-seed run under it (`notebooks/30_*`) gives 95.24% ± 0.93%, close to the random-split number, although that run still carries the negative-shape pairing issue noted in Section 5.2 and will be repeated. Third, the primary negatives so far are dinucleotide shuffles, which preserve composition exactly; the measured separability and the bio-branch effect therefore have to be re-confirmed on the real genomic negatives, which are now prepared and verified. Fourth, the controlled ablation uses a single seed at a 15-epoch budget, so the bio-branch effect needs multi-seed confirmation and a no-bio variant trained to convergence. Fifth, paralog identification is still unsolved at 101 bp resolution. The classical-baseline numbers that were previously missing have since been generated and appear in Table 2 and Section 3.7.

### 5.5 Performance ceiling
Binary accuracy levels off near 96%. That is about what we would expect if a 101 bp window, read through sequence, shape, and biochemical context together, already holds most of the information the task can use. Pushing past it will probably need longer context, a chromatin or accessibility signal such as ATAC-seq, or quantitative binding affinities in place of binary peak calls.

---

## 6. Conclusion
We built a multimodal tri-branch model (G-CMAB) that predicts SP-family TF binding by fusing DNABERT-2 sequence embeddings, DNAshape geometry, and biochemical features through bidirectional cross-modal attention and a multi-scale CNN. After moving from per-paralog to binding-vs-non-binding classification — a change the data forced rather than one we chose freely — the model reaches 96.35% accuracy. It beats a sequence-only transformer mainly by rejecting GC-rich negatives, and it stays interpretable through SHAP and BED-track export. The open directions are generalization to other TF families and cell types, adding chromatin-accessibility data, and returning to paralog discrimination with longer genomic context.
