...
Under the binary formulation, and evaluated against dinucleotide-shuffled negatives, the multimodal G-CMAB model reaches 96.35% accuracy and macro-F1 0.9501 (Table 2), against 76.24% for the fine-tuned sequence-only DNABERT-2. Most of the gain is in negative rejection. Negative recall rises from 0.324 in the sequence-only model to 0.886, and Negative precision from 0.547 to 0.966; SP-Positive recall reaches 0.990. The pattern holds across every sequence-only method we tried: the fine-tuned transformer (76.2%) and the best classical k-mer model (80.7%) both stall at a Negative recall of 0.32–0.38, while the full multimodal model reaches 0.886. Section 4.4 and Section 5.2 work through which components actually account for this difference, and the answer is less tidy than "shape plus attention".
...
### 5.4 Limitations
Several limitations bound the present claims. First, the data come from a single cell line (HepG2) and a single TF family (the three SP paralogs), so generalization to other cell types and families is untested. Second, the negatives are dinucleotide-shuffled — a standard composition-matched control that preserves mono- and dinucleotide frequencies while destroying motif structure; performance against real GC-matched genomic background is a harder problem and is the focus of ongoing follow-up work (Section 6), so the present 96.35% should be read as a benchmark result rather than as expected genome-wide performance. Third, the headline split is a random group split that keeps each sequence with its reverse complement, which removes augmentation leakage but not genomic-proximity or homology leakage; a chromosome-holdout protocol is implemented (`notebooks/30_*`) for that follow-up. Fourth, the controlled ablation uses a single seed at a 15-epoch budget, so the component effects warrant multi-seed confirmation. Fifth, paralog identification is still unsolved at 101 bp resolution.

### 5.5 Performance ceiling
Binary accuracy on the dinucleotide-shuffled benchmark levels off near 96%, consistent with a 101 bp window holding most of what this task needs once sequence, shape, and biochemical context are all used. Pushing past it — and onto harder real-genomic negatives — will probably need longer context, a chromatin or accessibility signal such as ATAC-seq, or quantitative binding affinities in place of binary peak calls.

---

## 6. Conclusion
We built a multimodal tri-branch model (G-CMAB) that predicts SP-family TF binding by fusing DNABERT-2 sequence embeddings, DNAshape geometry, and biochemical features through bidirectional cross-modal attention and a multi-scale CNN. After moving from per-paralog to binding-vs-non-binding classification — a change the data forced rather than one we chose freely — the model reaches 96.35% accuracy against dinucleotide-shuffled negatives, beating a sequence-only transformer mainly by rejecting those negatives, and stays interpretable throughout via SHAP and BED-track export. The clearest next phase is extending the approach to real GC-matched genomic negatives — a substantially harder setting — with further development of the shape and biochemical branches. Other open directions are generalization to additional TF families and cell types, adding chromatin-accessibility data, and revisiting paralog discrimination with longer genomic context.
...
Per the chosen policy, every quantitative result above is copied from a saved report (`classification_report.txt`, `{onehot,tfidf}_report.txt`, or `ablation_results.csv`) actually present in the repository. Status of previously-disputed numbers: (1) **Classical baselines** have now been re-run — the best classical model is TF-IDF logistic regression at **80.73 %** (the README's "78.50 % SVM" was close but not the best; SVM-TF-IDF was 80.43 %). (2) The **sequence-only DNABERT-2** saved report shows **76.24 %** (the README's 89.05 % is not backed by any saved report and is not used). (3) The proposed-model **96.35 %** is corroborated by the saved binary report in `outputs_gcmab_binary` — whose model file (`best_gcmab_binary.pt`) and output naming match the **tri-branch** script 23 (the reproducible trainer is `notebooks/28_*`); the "Script 19" string inside that report is a stale hard-coded title, not the architecture. (4) The exclusive-peak counts were corrected to the on-disk values (6,815 / 1,696 / 8,914). (5) **Phasing.** Real-genomic-negative experiments have already been run internally (saved under `figures/rerun/`: full model 79.6 % random-split / 79.4 % ± 0.8 % chromosome-holdout / 77.9 % real-negative eval; classical TF-IDF 80.73 %; sequence-only 78.93 %; and `artifact_check.txt` records an 18-point dinuc→genomic drop with the bio branch reversing sign in the genomic ablation). These are **deliberately deferred to a Phase-2 follow-up**: this Phase-1 manuscript is scoped to the dinucleotide-shuffled benchmark, and Sections 5.2/5.4/6 reference the genomic setting only as ongoing/future work, never as a reported result — so the manuscript neither overclaims genome-wide performance nor reports the deferred numbers. (6) **Table 2 consistency item.** The classical baselines were produced by `notebooks/27_*`, which defaults to the genomic GC-matched negatives (verified: TF-IDF logistic regression = 80.73 %; the dinucleotide value is 80.55 %). For a fully dinuc-consistent Table 2, either re-run script 27 on the dinucleotide negatives or footnote the row. (7) Resolve the `[VERIFY]` citations in `PAPER_SUPPLEMENT.md`.
